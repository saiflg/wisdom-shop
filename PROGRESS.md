...
- **The machine never guesses.** Where the answer is unambiguous it marks.
  Everywhere else it awards nothing *and says so*, so a human looks before
  the paper is finished. A zero nobody reviewed is indistinguishable from a
  zero the student earned, and that is the failure worth engineering against:
  an essay is never auto-marked even when blank; a short answer matching
  nothing the teacher listed is recorded zero **and** flagged, because "5cm"
  against an accepted "5 cm" is a right answer the teacher didn't think to
  spell out; two selections on a single-answer question are refused rather
  than resolved by picking one.
- **Short answers are compared on case and whitespace only.** No stemming, no
  fuzzy distance, no stripping punctuation. Every one of those eventually
  marks a wrong answer right, and marking a wrong answer right is worse than
  sending a right one to a teacher.
- **Multiple choice is all-or-nothing**, stated plainly to the student on the
  paper. Partial credit needs a policy the teacher chose — how much for three
  of four, what a wrong tick costs — and inventing one here would silently
  apply it to every school using the product.
- **An exam question is a snapshot of the bank question**, copied when the
  paper is built. Same rule as a published TermResult and a Payslip: a teacher
  correcting a typo next term must not change the questions a student already
  answered or the marks given for them. Retiring a bank question is a soft
  delete and the paper's copy survives it.
- **Two clocks, deliberately separate.** The window (`opensAt`/`closesAt`) is
  the school's; the duration is the student's, counted from when *they* start.
  A student starting five minutes before close gets five minutes, not their
  full hour — the alternative lets a class start late and sit past the end of
  the school day. Their deadline is computed once and stored, so an
  administrator editing the duration mid-morning cannot shorten a clock
  already running.
- **A dead laptop must not cost a child their paper.** Answers are saved one
  request at a time as they are given, so a dropped connection costs one
  question. Starting again resumes the same attempt on the same clock rather
  than refusing. And a paper whose time ran out while nobody was looking is
  collected and marked from what was saved, rather than sitting at
  IN_PROGRESS forever.
- **The countdown on screen is advisory.** It counts down locally from a
  number the server sent; the server decides what was in time. A clock a
  student could change by editing their own machine must never be the one
  that matters.
- **`@@unique([examId, studentProfileId])`** — one attempt per paper,
  enforced by the database rather than by a check the service could race
  with itself on.
- **The answer key leaves through exactly one function**, which rebuilds each
  question from named fields rather than deleting the key from the row. A
  `delete` would mutate an object a staff caller still holds, and a field
  added to the model later would be spread straight through to students by
  code nobody remembered to update. Naming what goes out means a new column
  is invisible until somebody decides it should not be. A student reviewing
  their own released result still never sees the key — papers get reused.
- **Marks are held back until released, and a paper waiting on a human is
  held back from the release too**, counted rather than let through with a
  hole in it. A released mark reaches the gradebook scaled, not copied.
- **AI drafts questions into the bank, never onto a paper.** A generated
  answer key that is wrong marks a whole class wrong, silently, all the way to
  a report card. Anything the normaliser cannot verify is internally
  consistent — a key naming an option that isn't there, a single-answer
  question with two right answers — is thrown away, and *the teacher is told
  what was dropped and why* rather than being left to count five against
  three. ESSAY is not offered: there is no key to generate, and a model asked
  for one writes something that reads like a mark scheme without being one.
